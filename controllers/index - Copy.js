...
document.querySelector("#btnThemNV").onclick = function () {
  ...
  //Kiểm tra dữ liệu nhan viên có hợp lệ hay không?
  var valid = true; // mặc định form là hợp lệ
  //Kiểm tra tài khoản
  // valid &=
  //   kiemTraTrungLap(nv.taiKhoan, "#tbTKNV", "Tài khoản") &
  //   kiemTraDoDai(nv.taiKhoan, "#tbTKNV", "Tài khoản", 4, 6) &
  //   kiemTraRong(nv.taiKhoan, "#tbTKNV", "Tài khoản");
  valid &= checkTaiKhoan();

  //Kiểm tra họ tên
  valid &=
    kiemTraTatCaKyTu(nv.hoTen, "#tbTen", "Họ và tên") &
    kiemTraRong(nv.hoTen, "#tbTen", "Họ và tên");

  //Kiểm tra email
  valid &=
    kiemTraEmail(nv.email, "#tbEmail", "Email") &
    kiemTraRong(nv.email, "#tbEmail", "Email");

  //Kiểm tra mật khẩu
  valid &=
    kiemTraMatKhau(nv.matKhau, "#tbMatKhau", "Mật khẩu", 6, 10) &
    kiemTraRong(nv.matKhau, "#tbMatKhau", "Mật khẩu");

  //Kiểm tra ngày làm
  valid &=
    kiemTraNgay(nv.ngayLam, "#tbNgay", "Ngày làm") &
    kiemTraRong(nv.ngayLam, "#tbNgay", "Ngày làm");

  //Kiểm tra lương cơ bản
  valid &=
    kiemTraGiaTri(nv.luongCB, "#tbLuongCB", "Lương cơ bản", 1000000, 20000000) &
    kiemTraRong(nv.luongCB, "#tbLuongCB", "Lương cơ bản");

  //Kiểm tra chức vụ
  valid &= kiemTraHopLe(nv.chucVu, "#tbChucVu", "Chức vụ");

  //Kiểm tra giờ làm
  valid &=
    kiemTraGiaTri(nv.gioLam, "#tbGiolam", "Giờ làm", 80, 200) &
  ...
  //Gọi hàm từ mangr nhân viên tạo ra html cho table
  renderTableNhanVien(mangNhanVien);
